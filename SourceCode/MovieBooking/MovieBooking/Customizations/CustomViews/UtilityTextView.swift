...
extension UITextViewDelegate {
    func utilityTextViewDidTouchNextButton(_ textView: UtilityTextView) {
        // Do nothing
    }
    
    func utilityTextViewDidTouchBackButton(_ textView: UtilityTextView) {
        // Do nothing
    }
}

class UtilityTextView: RichTextView {
    ...
    var enableNextBackBarButtons: Bool = false {
        didSet {
            if self.enableNextBackBarButtons && self.nextButtonItem == nil && self.backButtonItem == nil {
                ...
                self.backButtonItem = UIBarButtonItem(image: UIImage(named: "ico_back"), style: .plain, target: self, action: #selector(UtilityTextView.didTouchBackButton))
                self.backButtonItem!.width = 30
                self.backButtonItem!.tintColor = UIColor.colorFromHexValue(kDefaultButtonTintColor)
                
                self.nextButtonItem = UIBarButtonItem(image: UIImage(named: "ico_next"), style: .plain, target: self, action: #selector(UtilityTextView.didTouchNextButton))
    ...
    func didTouchDoneBarButton(_ doneButton: UIBarButtonItem) {
